...
def update_index(cnt_que, index): # 在pred_golden插入一个数后需要更新cnt_que里大于等于index（插入的位置）的值
    ...
    for each in history[start:]:
        ...
        if index == 0:  # 边界特殊处理
            if pred_golden[0] - new > pred_golden[1] - pred_golden[0]:  # 新增待定黄金点
                ...
                pred_golden[index] = (1 - WEIGHT_DECAY) * new + WEIGHT_DECAY * pred_golden[index] # 滑动平均更新pred_golden里的值
            ...
            if new - pred_golden[index - 1] > pred_golden[index - 1] - pred_golden[index - 2]:  # 新增待定黄金点
                ...
                pred_golden[index - 1] = (1 - WEIGHT_DECAY) * new + WEIGHT_DECAY * pred_golden[index - 1] # 滑动平均更新pred_golden里的值
                if len(cnt_que) == MAX_QUE_SIZE:
                    cnt_que.pop()
                cnt_que.insert(0, index - 1)
        elif pred_golden[index] - new < (pred_golden[index] - pred_golden[index - 1]) / 3.0: # 落到右1/3区域
            pred_golden[index] = (1 - WEIGHT_DECAY) * new + WEIGHT_DECAY * pred_golden[index]
            if len(cnt_que) == MAX_QUE_SIZE:
                cnt_que.pop()
            cnt_que.insert(0, index)
        elif new - pred_golden[index - 1] < (pred_golden[index] - pred_golden[index - 1]) / 3.0: # 落到左1/3区域
            pred_golden[index - 1] = (1 - WEIGHT_DECAY) * new + WEIGHT_DECAY * pred_golden[index - 1]
            if len(cnt_que) == MAX_QUE_SIZE:
                cnt_que.pop()
            cnt_que.insert(0, index - 1)
        else:  # 落到中间1/3区域，新增待定黄金点
            pred_golden.insert(index, new)
            if len(cnt_que) == MAX_QUE_SIZE:
                cnt_que.pop()
...
def main():
    if len(history) == 0: # 第一步策略
        ...
        if len(candidates) < 2: # 如果统计结果没有前两个值
